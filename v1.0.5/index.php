...
$app->post('/event', 'authenticate', function () use ($app) {
            ...
            if($result){
                ...
                while ($event = $result->fetch_assoc()) {
                    ...
                    $tmp['event_organisers'] = $event["evnt_organisers"];
                ...
                while ($event2 = $result2->fetch_assoc()) {
                    ...
                    $tmp['event_organisers'] = $event2["evnt_organisers"];
                    array_push($response["events"], $tmp);
                }
...
$app->get('/event/:event_id', 'authenticate', function ($event_id) {
            ...
            if($result){
                ...
                while ($event_detail = $result->fetch_assoc()) {
                    ...
                    $response['event_pincode'] = $event_detail["evnt_pincode"];
...
$app->get('/event2', 'authenticate', function () {
            global $user_id;
            $response = array();
            $db = new DbHandler();
...
$app->get('/event2/favourite', 'authenticate', function () {
            global $user_id;
            $response = array();
            $db = new DbHandler();
...
$app->get('/event2/:event_id', 'authenticate', function ($event_id) {
            global $user_id;
            $response = array();
            $db = new DbHandler();
